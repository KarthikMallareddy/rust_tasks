...
import type { VaultEntry, DerivedKey } from "../src/types.js"
...
test("AES-GCM: Detects tampered ciphertext", async () => {
  ...
  await assert.rejects(
    async () => await decrypt(tamperedEncrypted, derivedKey),
    /Decryption failed/,
    "Tampered ciphertext should throw error"
  )
})
...
test("AES-GCM: Rejects decryption with wrong key", async () => {
  ...
  await assert.rejects(
    async () => await decrypt(encrypted, wrongKey),
    /Decryption failed/,
